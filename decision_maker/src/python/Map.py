...
class Map():
    def __init__(self, node:Node):
        ...
        # Calibration values
        camera_info_topic = self.node.declare_parameter('/decision_maker/cameraInfo_topic', '/camera/camera_info').value
        # camera_info = self.wait_for_message(camera_info_topic, CameraInfo, timeout=5)
        camera_info = wait_for_message(self.node, CameraInfo, camera_info_topic)
    ...
    def setCameraBaseTf(self, t_camera_base, quat_camera_base):
        ...
        """
        self.rot_camera_base = Rotation.from_quat([float(quat_camera_base.x), float(quat_camera_base.y), float(quat_camera_base.z), float(quat_camera_base.w)])
        self.t_camera_base = [float(t_camera_base.x), float(t_camera_base.y), float(t_camera_base.z)]
        self.T_camera_base = sp.SE3(self.rot_camera_base.as_matrix(), self.t_camera_base)
...
def wait_for_message(node : Node, msg_type, topic, time_out=5):
    import time
    class _wfm(object):
